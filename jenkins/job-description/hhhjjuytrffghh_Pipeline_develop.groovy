...
def job_config = [
        job: [
                name: "hhhjjuytrffghh_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'hhhjjuytrffghh_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/hhhjjuytrffghh_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = lib.de.allianz.hhhjjuytrffghh.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "hhhjjuytrffghh_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
